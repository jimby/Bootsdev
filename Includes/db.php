...
class CustDB extends mysqli {
   ...
   /********** customer functions **********/ 
    ...
    function insert_customer($customerID,$lname,$fname,$mi,$name,$street,$city,$province,$country,$postal_code,$phone,$email,$ship_name,$ship_street,$ship_city,$ship_province,$ship_country,$ship_postal_code,$ship_phone,$ship_email) {
        ...
        $this->query ("INSERT INTO customers   (city,country,email,fname,lname,mi,name,phone,postal_code,province,ship_city,ship_country,ship_email,ship_name,ship_phone,ship_postal_code,ship_province,ship_street,street)  VALUES ('$city','$country','$email','$fname','$lname','$mi','$name','$phone','$postal_code','$province','$ship_city','$ship_country','$ship_email','$ship_name','$ship_phone','$ship_postal_code','$ship_province','$ship_street','$street')");
    }
}
...
/* inventory functions****************************************************************/
    ...
    function insert_item($warehouseID,$product,$cost,$qoh,$DatePlanted,$DateReady,$SeedTransplant,$LotNumber,$description,$pix_path,$sci_name) {
	    ...
	    $pix_path = $this->real_escape_string($pix_path);
	    $sci_name = $this->real_escape_string($sci_name);
        $this->query ("INSERT INTO inventory
            (warehouse_id,product,cost,qoh,DatePlanted,DateReady,SeedTransplant,LotNumber,description,pix_path,sci_name)
            VALUES
            ('" .$warehouseID. "','" .$product. "','" .$cost. "','" .$qoh. "','" .$DatePlanted. "','" .$DateReady. "','" .$SeedTransplant. "','" .$LotNumber. "','" .$description. "','" .$pix_path. "','" .$sci_name. "')");
    }
    public function update_inventory($itemID,$warehouseID,$product,$cost,$qoh,$DatePlanted,$DateReady,$SeedTransplant,$LotNumber,$description,$pix_path,$sci_name) {
	$itemID=(int)$itemID;
	$warehouseID = intval($warehouseID);
        $product = $this->real_escape_string($product);
        $cost = $this->real_escape_string($cost);
    ...
    public function get_item_by_product($ItemName) {
        $ItemName = $this->real_escape_string($ItemName);
	$string="select i.product,i.cost,i.qoh,i.LotNumber,i.description,i.pix_path,w.wlocation from inventory as i,warehouses as w where i.warehouse_id=w.id and product like "."  '%$ItemName%'";	
	//  $string="select * from inventory i, wname,wlocation from warehouse w where i.warehouse_id=i.id and product like "." '%$ItemName%'";	
         //$string="select * from inventory i, wname,wlocation from warehouse w where i.warehouse_id=i.id and i.product like "." '%$ItemName%'";
        return $this->query($string);
    }
        
    public function get_item_by_item_id($itemID) {
        //return $this->query("SELECT id,warehouse_id,qoh, product, cost,DatePlanted,DateReady,SeedTransplant,LotNumber,description,pix_path,sci_name FROM inventory WHERE id = " . $itemID);
	return $this->query("SELECT * FROM inventory WHERE id = " . $itemID);	
    }
    public function get_items_by_warehouse_id($warehouseID) {
        	//return $this->query("SELECT id,warehouse_id,qoh, product, cost,DatePlanted,DateReady,SeedTransplant,LotNumber,description,pix_path,sci_name FROM inventory WHERE warehouse_id = $warehouseID order by product");
    return $this->query("SELECT * FROM inventory WHERE warehouse_id = $warehouseID order by product");
...
/***** BEGIN warehouse**********************************/
    public function get_warehouse_id_by_location($mlocation) {
	    ...
	    $query="select id from warehouses where wlocation like "." '%$mlocation%'";	 	
  ...
  class OrdersDB extends mysqli {
    ...
    private function __construct() {
      ...
      public function update_Orders($orderid,$customerid,$onumber,$odate,$custOrderNo,
              $custOrderDt,$oSubT,$oTax,$oShipChg,$oTot) {
         $orderid = intval($orderid); 
        $customerid= intval($customerid);
        $onumber = $this->real_escape_string($onumber);
        $odate = $this->real_escape_string($odate);
        $custOrderNo = $this->real_escape_string($custOrderNo);   
        $custOrderDt = $this->real_escape_string($custOrderDt);
        $oSubT = $this->real_escape_string($oSubT);
        $oTax = $this->real_escape_string($oTax);
        $oShipChg = $this->real_escape_string($oShipChg);
        $oTot = $this->real_escape_string($oTot);
        ...
        function insert_Orders($customerid,$onumber,$odate,$custOrderNo,$custOrderDt,$oSubT,
                $oTax,$oShipChg,$oTot) {
        $customerid= intval($customerid);
        $onumber = $this->real_escape_string($onumber);
        $odate = $this->real_escape_string($odate);
        ...
        $oTot = $this->real_escape_string($oTot);
         $this->query ("INSERT INTO orders
            (customerid,onumber,odate,custOrderNo,custOrderDt,oSubT,
            oTax,oShipChg,oTot) VALUES
            ('$customerid','$onumber','$odate','$custOrderNo','$custOrderDt','$oSubT',
                '$oTax','$oShipChg','$oTot')");
    }
